...
struct CBMType {
    ...
    union {
        ...
        struct {
            const CBMType *elem;
            int direction;
        } channel; // CHANNEL (0=bidi, 1=send, 2=recv)
        struct {
        ...
        struct {
            const CBMType *elem;
        } reference; // REFERENCE / RVALUE_REF
        struct {
        ...
        struct {
            const CBMType *operand;
        } keyof; // KEYOF
        struct {
            const char *expr;
        } typeof_query; // TYPEOF_QUERY (referenced expression text)
        struct {
