...
static void reg_method(CBMTypeRegistry *reg, CBMArena *arena, const char *recv_qn,
                       const char *method_name, const CBMType *ret_type) {
    ...
    rf.signature =
        cbm_type_func(arena, NULL, NULL, ret_type ? (const CBMType *[]){ret_type, NULL} : NULL);
...
static void reg_type_with_fields(CBMTypeRegistry *reg, CBMArena *arena, const char *qn,
                                 const char *short_name, const char **fnames,
                                 const CBMType **ftypes) {
    CBMRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = qn;
    rt.short_name = short_name;
...
static void reg_func(CBMTypeRegistry *reg, CBMArena *arena, const char *qn, const char *short_name,
                     const CBMType *ret_type) {
    CBMRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.min_params = -1;
    rf.qualified_name = qn;
...
void cbm_cpp_stdlib_register(CBMTypeRegistry *reg, CBMArena *arena) {
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique", cbm_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared", cbm_type_named(arena, sptr_qn));
    ...
    const char *pair_qn = "std.pair";
    {
        ...
        const CBMType **pair_field_types =
            (const CBMType **)cbm_arena_alloc(arena, 3 * sizeof(const CBMType *));
    ...
    reg_func(reg, arena, "std.make_pair", "make_pair", cbm_type_named(arena, "std.pair"));
    ...
    reg_func(reg, arena, "std.minmax", "minmax", cbm_type_named(arena, "std.pair"));
    ...
    reg_func(reg, arena, "std.mismatch", "mismatch", cbm_type_named(arena, "std.pair"));
    ...
    reg_func(reg, arena, "std.allocate_shared", "allocate_shared", cbm_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size",
             cbm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all",
             cbm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.absolute", "absolute", cbm_type_named(arena, fspath_qn));
    reg_func(reg, arena, "std.filesystem.canonical", "canonical", cbm_type_named(arena, fspath_qn));
    reg_func(reg, arena, "std.filesystem.relative", "relative", cbm_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data",
               cbm_type_pointer(arena, cbm_type_builtin(arena, "char")));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit", cbm_type_named(arena, "std.vector"));
    ...
    reg_func(reg, arena, "spdlog.get", "get", cbm_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent",
               cbm_type_pointer(arena, cbm_type_named(arena, qobj_qn)));
