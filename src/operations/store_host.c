...
static cbm_store_t *resolve_store_fallback_scan(const char *project) {
    ...
    while ((entry = cbm_readdir(d)) != NULL) {
        ...
        size_t len = strlen(n);
        if (!cbm_store_host_is_project_db_file(n, len))
            continue;
...
static bool reserve_unique_corrupt_pending(const char *path, char *pending, size_t pending_size,
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        ...
        if (bw <= 0 || (size_t)bw >= backup_size || pw <= 0 || (size_t)pw >= pending_size)
            return false;
...
static bool quarantine_corrupt_store(cbm_store_host_t *host, const char *project, const char *path,
...
static bool mutation_begin(cbm_store_host_t *host, const char *project, bool nonblocking) {
    if (!host->mutation_begin)
        return true;
    if (nonblocking)
        return host->mutation_try_begin &&
               host->mutation_try_begin(host->mutation_context, project);
...
static void mutation_end(cbm_store_host_t *host, const char *project) {
    if (host->mutation_end)
        host->mutation_end(host->mutation_context, project);
}

static cbm_store_host_t *host_alloc(void) {
    cbm_store_host_t *host = calloc(1, sizeof(*host));
    if (host)
        host->owns_store = true;
    return host;
}
...
cbm_store_host_t *cbm_store_host_new(const char *store_path) {
    ...
    if (store_path) {
        ...
        host->store = cbm_store_open_memory();
    return host;
}
...
void cbm_store_host_free(cbm_store_host_t *host) {
    if (!host)
        return;
    if (host->owns_store && host->store)
        cbm_store_close(host->store);
...
const char *cbm_store_host_current_project(const cbm_store_host_t *host) {
    return host ? host->current_project : NULL;
}
...
void cbm_store_host_detach_project(cbm_store_host_t *host, const char *project) {
    if (!host || !project || !project[0])
        return;
    if (host->current_project && strcmp(host->current_project, project) == 0)
        cbm_store_host_invalidate(host);
}

void cbm_store_host_evict_idle(cbm_store_host_t *host, int timeout_s) {
    if (!host || !host->store || host->store_last_used == 0)
        return;
    if ((time(NULL) - host->store_last_used) < timeout_s)
        return;
    cbm_store_host_invalidate(host);
...
bool cbm_store_host_release_pristine_memory_store(cbm_store_host_t *host) {
    ...
    if (!host || !host->owns_store || !host->store || host->current_project ||
        host->store_last_used != 0 || db_path)
        return false;
    cbm_store_close(host->store);
    host->store = NULL;
    return true;
...
cbm_store_t *cbm_store_host_resolve(cbm_store_host_t *host, const char *project,
    ...
    if (recovery_status)
        *recovery_status = CBM_OPERATION_STORE_RECOVERY_NONE;
    if (!host || !project)
        return NULL;
    host->store_last_used = time(NULL);
    if (host->current_project && strcmp(host->current_project, project) == 0 && host->store)
        return host->store;
    cbm_store_host_invalidate(host);
    char path[CBM_SZ_1K];
    ...
    if (host->store && !cbm_store_check_integrity(host->store)) {
        ...
        host->store = NULL;
        bool acquired =
            mutation_already_held || mutation_begin(host, project, nonblocking_recovery);
        if (!acquired) {
            if (recovery_status)
                *recovery_status = host->mutation_try_begin
        ...
        cbm_integrity_verdict_t verdict =
            host->store ? cbm_store_check_integrity_verdict(host->store) : CBM_INTEGRITY_TRANSIENT;
        if (verdict == CBM_INTEGRITY_TRANSIENT) {
            ...
            host->store = NULL;
            if (recovery_status)
                *recovery_status = CBM_OPERATION_STORE_RECOVERY_BUSY;
            if (!mutation_already_held)
                mutation_end(host, project);
            return NULL;
        }
...
static int collect_db_project_names(char *out, size_t out_sz) {
    ...
    while ((entry = cbm_readdir(d)) != NULL) {
        ...
        char iname[CBM_SZ_1K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, n);
        if (!cbm_store_host_db_internal_project_name(full_path, iname, sizeof(iname), NULL))
            continue;
        size_t need = strlen(iname) + 2 + (count ? 1u : 0u);
        if (offset + need + 1 > out_sz)
            break;
        if (count)
            out[offset++] = ',';
...
char *cbm_store_host_error(const char *project) {
    if (!project) {
        return host_strdup(
            ...
            "\\\"project\\\" argument, e.g. {\\\"project\\\":\\\"<name from list_projects>\\\"}. "
    ...
    if (count > 0) {
        snprintf(buf, sizeof(buf),
                 "{\"error\":\"project not found or not indexed\",\"hint\":\"Use list_projects to "
                 "see all indexed projects, then pass it as the \\\"project\\\" "
                 "argument.\",\"available_projects\":[%s],\"count\":%d}",
                 projects, count);
    } else {
        snprintf(buf, sizeof(buf),
                 "{\"error\":\"project not found or not indexed\",\"hint\":\"No projects indexed "
                 "yet. Call index_repository first.\"}");
    }
