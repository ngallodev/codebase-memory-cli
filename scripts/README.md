...
| **lint** | `lint.sh` | clang-tidy + parallel Cppcheck + clang-format (+ no-skips policy). `--ci` = the CI gate set (no clang-tidy). Drives the same make targets as `make lint`/`lint-ci`; override Cppcheck workers with `CPPCHECK_JOBS=1..N`. |
