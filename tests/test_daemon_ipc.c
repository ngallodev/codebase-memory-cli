...
#ifndef _WIN32
static int test_daemon_ipc_posix_uid_map_single_uid_parse_issue1830(void);
static int test_daemon_ipc_posix_overflow_ancestor_tolerated_only_in_single_uid_ns_issue1830(void);
static int test_daemon_ipc_posix_single_uid_userns_real_smoke_issue1830(void);
#endif
#ifdef _WIN32
static int test_daemon_ipc_windows_sid_trust_accepts_local_admin_rejects_foreign_500(void);
#endif
...
TEST(daemon_ipc_posix_single_uid_userns_real_smoke_issue1830) {
    ...
    if (child == 0) {
        ...
        /* Some kernels leave the caller at overflowuid after installing a
         * nonzero-only map; that is not an executable 1:1 current-user map. */
        if (geteuid() != host_uid) {
            _exit(2);
        }
